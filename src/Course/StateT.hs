...
instance Functor k => Functor (StateT s k) where
  (<$>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  f <$> StateT stok = StateT $ \s -> (g <$> (stok s))
    where g = \(a, s) -> (f a, s)
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  -- StateT kab <*> StateT ka = StateT $ \s -> (kab s) >>=
  --   (\(fab, s') -> (ka s') >>=
  --     (\(a, s'') -> pure (fab a, s'')))
...
distinct' ::
  ...
  List a
  -> List a
distinct' xs = eval' (filtering notSeenYet xs) S.empty
  where notSeenYet x = state' $ \s -> (S.notMember x s, S.insert x s)
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF xs = evalT (filtering notSeenYet xs) S.empty
  where notSeenYet x = StateT $ \s -> if x > 100 then Empty else Full (S.notMember x s, S.insert x s)
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
-- We do a flatMap, each element in the first optional, is mapped to
-- that functional fmap'd with a. The variable f' is actually
-- an Optional (a -> b), by doing (f' <$>) we can extract its contents.
...
instance Monad k => Monad (OptionalT k) where
  (=<<) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  f =<< OptionalT ka = OptionalT $ ka >>= (\o -> case o of
    Empty -> pure Empty
...
instance Monad (Logger l) where
  (=<<) ::
    ...
    -> Logger l a
    -> Logger l b
  falb =<< Logger l0 a0 = Logger (l0 ++ l1) a1
    where Logger l1 a1 = falb a0
...
-- Previously StateT internal function took in state and gave Optional (a, state)
-- Now we want function to take in state and give OptionalT Logger logs (a, state)
-- State previously a simple set. StateT now is a function that takes a state and gives
-- OptionalT with Logger monad of Optional next (value, state).
distinctG xs = runOptionalT $ evalT (filtering (StateT . notSeenYet) xs) S.empty
  where
  notSeenYet x
    | x > 100 = \_ -> OptionalT $ log1 (fromString ("aborting > 100: " P.++ show x)) Empty
    | even x = \s -> OptionalT $ log1 (fromString ("even number: " P.++ show x)) (Full (S.notMember x s, S.insert x s))
    | otherwise = \s -> OptionalT $ pure (Full (S.notMember x s, S.insert x s))
