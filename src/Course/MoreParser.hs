...
-- True
-- For first argument traverse takes function (Char -> Parser Char
-- For second argument traverse takes Chars ~= List Char
-- For return traverse returns Parser List Char ~= Parser Chars
string ::
  Chars
  -> Parser Chars
...
hex ::
  Parser Char
hex = let hexP = replicateA 4 (satisfy isHexDigit) in do
  hextet <- hexP
  return (chr $ (readHex hextet) ?? 0)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
-- Apply parsed character to all in list
satisfyAll xs = satisfy (\c -> and (($ c) <$> xs))
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny xs = satisfy (\c -> or (($ c) <$> xs))
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma c1 c2 p = betweenCharTok c1 c2 contentP
  where contentP = sepby p (charTok ',')
