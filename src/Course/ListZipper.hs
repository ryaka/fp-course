...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil x r) = let (y:.ys) = (reverse (x:.r)) in (ListZipper ys y Nil)
...
moveRightLoop (ListZipper l x Nil) = let (y:.ys) = (reverse (x:.l)) in (ListZipper Nil y ys)
...
moveLeftN n (ListZipper l x r)
  | n == 0 = isZ $ ListZipper l x r
  | n > (length l) = isNotZ
  | n < 0 = moveRightN (-n) (ListZipper l x r)
  | otherwise = let (y:.ys) = l in moveLeftN (n - 1) (ListZipper ys y (x:.r))
...
moveRightN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveRightN n (ListZipper l x r)
  | n == 0 = isZ $ ListZipper l x r
  | n > (length r) = isNotZ
  | n < 0 = moveLeftN (-n) (ListZipper l x r)
  | otherwise = let (y:.ys) = r in moveRightN (n - 1) (ListZipper (x:.l) y ys)
...
moveLeftN' n (ListZipper l x r)
  ...
  | n < 0 = moveRightN' (negate n) (ListZipper l x r)
...
moveRightN' n (ListZipper l x r)
  | n == 0 = Right (ListZipper l x r)
  | n < 0 = moveLeftN' (negate n) (ListZipper l x r)
...
-- /Tip:/ Use `zipWith`
  (<*>) ::
    ...
    -> ListZipper a
    -> ListZipper b
  ListZipper fl fx fr <*> ListZipper l x r = ListZipper (zipWith g fl l) (fx x) (zipWith g fr r)
    where g f y = f y
...
-- >[2,1] >3< [4,5]<
-- [[3,2,1] >4< [5],[4,3,2,1] >5< []]
-- tuple, first value is yielded value, second is the next value to generate from
...
instance Extend ListZipper where
  (<<=) ::
    ...
    -> ListZipper a
    -> ListZipper b
  f <<= lz = ListZipper (f <$> allLefts) (f lz) (f <$> allRights)
    where
      next moveFn = \x -> let olz = moveFn x in
        case olz of
          MLZ Empty -> Empty
          MLZ (Full lz') -> Full (lz', lz')
      allLefts = unfoldr (next moveLeft) lz
      allRights = unfoldr (next moveRight) lz
...
instance Extend MaybeListZipper where
  (<<=) ::
    ...
    -> MaybeListZipper b
  _ <<= MLZ Empty = isNotZ
  f <<= MLZ (Full lz) = isZ $ (\x -> f $ isZ x) <<= lz
